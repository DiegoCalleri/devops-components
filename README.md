...
Передайте `env-content`, и workflow подставит содержимое в SSH script через heredoc, запишет `.env` на сервере с `chmod 600`, а затем запустит `docker compose`.
...
- Не добавляйте в `env-content` / `ENV_CONTENT` строку, равную `__DEVOPS_COMPONENTS_ENV_EOF__` — это служебный heredoc delimiter.
